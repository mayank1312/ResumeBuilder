  ...
  const calculateCompletion = (resume) => {
    ...
    const fetchAllResumes=async()=>{
        try{
           ...
           const resumesWithCompleteion=response.data.map(resume=>({
            ...resume,
            completion:calculateCompletion(resume)

           }))
           setAllResume(resumesWithCompleteion)
        }catch(error){
    ...
    const handdleDeleteResume=async()=>{
  ...
  return (
    <DashboardLayout>
           <div className={styles.container}>
            <div className={styles.headerWrapper}>
                <div>
                    ...
                    <p className={styles.headerSubtitle}>
                        {allResume.length>0 ? `You have ${allResume.length} resumes${allResume.length !== 1 ? "s":" "}` : 
            ...
            {!loading && allResume.length===0 && (
                <div className={styles.emptyStateWrapper}>
                    ...
                    <p className={styles.emptyText}> You havent created any resume yet </p>
           ...
           <Modal isOpen={showDeleteConfirm} onClose={()=>setShowDeleteConfirm(false)}  title="Confirm Deletion" 
                showActionBtn actionBtnText="Delete" actionBtnClassName="bg-red-600 hover:bg-red-700" onActionClick={handdleDeleteResume}
            >
            <div className="p-4">
